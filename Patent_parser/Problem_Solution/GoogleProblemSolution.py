...
class GoogleProblemSolution(ProblemSolution):
    def __init__(self) -> None:
        super().__init__()

    def get_problem(self, patent_id: int) -> List[str]:
        problems = []
        result = client.query(f'SELECT Description, Abstract FROM db_patents.google_patents WHERE Id = {patent_id}')
    ...
    def get_solution(self, patent_id: int) -> str:
        return client.query(f'SELECT Title FROM db_patents.google_patents WHERE Id = {patent_id}').result_rows[0][0]
